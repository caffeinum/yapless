...
- **bluetooth input hijack**: with no `--input`, yapless opens whatever is the system default — if that's AirPods, starting IO forces the bidirectional bluetooth link (24kHz input stream), which degrades playback and steals the AirPods away from any iPhone/iPad using them. reading device properties (`--list-inputs`) is safe; only starting IO triggers it. there is no CoreAudio API for "never use this device for input", and `AVAudioSession` (which has `bluetoothHighQualityRecording`) is unavailable on macOS. `Config.swift` has no audio/input section, so the preference can't be persisted — `--input` is CLI-only.
